...
from .errors import ConfigFileNotFoundError, ProfileNotSupportedError, SchemaValidationError
...
class ConfigLoader:
    ...
    def load_global_config(self, file_path: str | Path | None = None) -> tuple[GlobalConfig, str]:
        if file_path is None:
            default_path = self.workspace_root / "config" / "global_config.json"
            if not default_path.exists():
                validate_global_config_data(_BUILTIN_GLOBAL_CONFIG, source="<builtin:global_config>")
                return GlobalConfig.from_dict(_BUILTIN_GLOBAL_CONFIG), "<builtin:global_config>"
    ...
    def load_board_profile(
        ...
        *,
        ...
        if file_path is not None:
            ...
            if not profile_name:
                raise ProfileNotSupportedError("board profile is required", field_path="product.board_profile")
            source_path = self.workspace_root / "config" / "boards" / f"{profile_name}.json"
    ...
    def load_case(self, file_path: str | Path, *, base_dir: str | Path | None = None) -> tuple[CaseSpec, str]:
    ...
    def load_fixture(self, file_path: str | Path, *, base_dir: str | Path | None = None) -> tuple[FixtureSpec, str]:
        source_path = self.resolve_path(file_path, base_dir=base_dir)
        raw = self._load_json(source_path)
    ...
    def resolve_path(
        ...
        *,
        ...
        if candidate.is_absolute():
            ...
            raise ConfigFileNotFoundError("configuration file does not exist", source=str(candidate))
    ...
    def _load_json(self, path: Path) -> dict[str, Any]:
        try:
            ...
            raise ConfigFileNotFoundError("configuration file does not exist", source=str(path)) from error
