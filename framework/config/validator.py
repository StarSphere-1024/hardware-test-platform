...
def _ensure_mapping(
    value: Any, *, field_path: str, source: str | None
) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise SchemaValidationError(
            "expected object", field_path=field_path, source=source
        )
    return value


def _require_field(
    mapping: Mapping[str, Any], key: str, *, field_path: str, source: str | None
) -> Any:
    if key not in mapping:
        raise SchemaValidationError(
            "missing required field", field_path=field_path, source=source
        )
    return mapping[key]
...
def _require_string(
    ...
    if not isinstance(value, str) or not value.strip():
        raise SchemaValidationError(
            "expected non-empty string", field_path=field_path, source=source
        )
    return value
...
def _optional_string(
    ...
    if value is not None and not isinstance(value, str):
        raise SchemaValidationError(
            "expected string", field_path=field_path, source=source
        )
...
def _optional_bool(
    ...
    if value is not None and not isinstance(value, bool):
        raise SchemaValidationError(
            "expected bool", field_path=field_path, source=source
        )
...
def _optional_int(
    ...
    if value is not None and (not isinstance(value, int) or isinstance(value, bool)):
        raise SchemaValidationError(
            "expected int", field_path=field_path, source=source
        )
...
def _optional_nullable_int(
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise SchemaValidationError(
            "expected int or null", field_path=field_path, source=source
        )
...
def _optional_number(
    ...
    if value is not None and (
        ...
        raise SchemaValidationError(
            "expected number", field_path=field_path, source=source
        )
...
def _optional_list_of_strings(
    ...
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise SchemaValidationError(
            "expected list of strings", field_path=field_path, source=source
        )
...
def validate_global_config_data(data: Any, *, source: str | None = None) -> None:
    ...
    _optional_string(
        ...
        field_path="product.default_board_profile",
        source=source,
    )
    _optional_string(
        product, "board_profile", field_path="product.board_profile", source=source
    )
    ...
    if runtime is not None:
        ...
        _optional_int(
            runtime_mapping,
            "default_timeout",
            field_path="runtime.default_timeout",
            source=source,
        )
        _optional_int(
            runtime_mapping,
            "default_retry",
            field_path="runtime.default_retry",
            source=source,
        )
        _optional_int(
            runtime_mapping,
            "default_retry_interval",
            field_path="runtime.default_retry_interval",
            source=source,
        )
        _optional_number(
            runtime_mapping,
    ...
    if observability is not None:
        observability_mapping = _ensure_mapping(
            observability, field_path="observability", source=source
        )
        _optional_bool(
            observability_mapping,
            "report_enabled",
            field_path="observability.report_enabled",
            source=source,
        )
        _optional_bool(
            observability_mapping,
            "dashboard_enabled",
            field_path="observability.dashboard_enabled",
            source=source,
        )
...
def validate_board_profile_data(data: Any, *, source: str | None = None) -> None:
    ...
    _optional_list_of_strings(
        root, "supported_cases", field_path="supported_cases", source=source
    )
    _optional_list_of_strings(
        root, "tools_required", field_path="tools_required", source=source
    )

    interfaces = root.get("interfaces")
    if interfaces is not None:
        interfaces_mapping = _ensure_mapping(
            interfaces, field_path="interfaces", source=source
        )
        for name, interface_value in interfaces_mapping.items():
            if not isinstance(name, str):
                raise SchemaValidationError(
                    "expected string key", field_path="interfaces", source=source
                )
            if isinstance(interface_value, list):
                if any(not isinstance(item, str) for item in interface_value):
                    raise SchemaValidationError(
                        "expected list of strings",
                        field_path=f"interfaces.{name}",
                        source=source,
                    )
                continue
            interface_mapping = _ensure_mapping(
                interface_value, field_path=f"interfaces.{name}", source=source
            )
            ...
            if raw_items is not None and (
                ...
                raise SchemaValidationError(
                    "expected list of strings",
                    field_path=f"interfaces.{name}.items",
                    source=source,
                )
            ...
            if description is not None and not isinstance(description, str):
                raise SchemaValidationError(
                    "expected string",
                    field_path=f"interfaces.{name}.description",
                    source=source,
                )
            metadata = interface_mapping.get("metadata")
            if metadata is not None:
                _ensure_mapping(
                    metadata, field_path=f"interfaces.{name}.metadata", source=source
                )
...
def validate_case_data(data: Any, *, source: str | None = None) -> None:
    ...
    if not isinstance(functions, list) or not functions:
        raise SchemaValidationError(
            "expected non-empty list", field_path="functions", source=source
        )
    for index, item in enumerate(functions):
        item_path = f"functions[{index}]"
        function_mapping = _ensure_mapping(item, field_path=item_path, source=source)
        _require_string(
            function_mapping, "name", field_path=f"{item_path}.name", source=source
        )
        _optional_bool(
            function_mapping,
            "enabled",
            field_path=f"{item_path}.enabled",
            source=source,
        )
        for key in ("timeout", "retry", "retry_interval"):
            _optional_int(
                function_mapping, key, field_path=f"{item_path}.{key}", source=source
            )
        _optional_number(
            function_mapping,
            "resource_lock_quarantine_seconds",
        ...
        _optional_list_of_strings(
            function_mapping,
            "resources",
            field_path=f"{item_path}.resources",
            source=source,
        )
        _optional_list_of_strings(
            function_mapping, "tags", field_path=f"{item_path}.tags", source=source
        )

    required_interfaces = root.get("required_interfaces")
    if required_interfaces is not None:
        _ensure_mapping(
            required_interfaces, field_path="required_interfaces", source=source
        )
...
def validate_fixture_data(data: Any, *, source: str | None = None) -> None:
    ...
    if (
        ...
        raise SchemaValidationError(
            "expected non-empty list of case references",
            field_path="cases",
            source=source,
        )
