...
class Scheduler:
    ...
    def _execute_task(
        ...
        if task.execution_mode not in {"sequential", "parallel"}:
            raise UnsupportedExecutionModeError(f"unsupported task execution mode: {task.task_id} -> {task.execution_mode}")
        ...
        if task.execution_mode == "parallel":
            child_results = self._execute_children_parallel(task, child_tasks, children_by_parent, context, plan_id)
        else:
            child_results = self._execute_children_sequential(task, child_tasks, children_by_parent, context, plan_id)
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if resources and lock_info.get("acquired"):
                    release_status = normalize_status(result.status) if result is not None else ResultStatus.FAILED.value
                    quarantine_seconds = self._resource_quarantine_seconds(task, context) if release_status == ResultStatus.TIMEOUT.value else 0
    ...
    def _execute_children_parallel(
        ...
        with ThreadPoolExecutor(max_workers=len(child_tasks), thread_name_prefix="fixture-scheduler") as executor:
            while pending or in_flight:
                if not stop_submitting:
                    ready_tasks = [
                        child for child in child_tasks
                        if child.task_id in pending and self._dependencies_satisfied(child, scheduler_completed)
                    ]
                    for ready_task in ready_tasks:
                        in_flight[executor.submit(self._execute_task, ready_task, children_by_parent, context, plan_id)] = ready_task
            ...
            if stop_submitting and pending:
                for task_id, pending_task in pending.items():
                    results_by_task_id[task_id] = self._build_aborted_result(pending_task, "aborted by stop_on_failure")

        return [results_by_task_id[task_id] for task_id in ordered_ids if task_id in results_by_task_id]
    ...
    def _set_attempt(self, context: ExecutionContext, task_id: str, attempt: int) -> None:
        with self._runtime_state_lock:
            context.runtime_state.setdefault("attempts", {})[task_id] = attempt

    def _dependencies_satisfied(self, task: ExecutionTask, completed_task_ids: set[str]) -> bool:
    ...
    def _resource_quarantine_seconds(self, task: ExecutionTask, context: ExecutionContext) -> float:
        value = task.payload.get("resource_lock_quarantine_seconds")
        if value is None:
            value = context.resolved_config.resolved_runtime.get("resource_lock_quarantine_seconds")
        if value is None:
    ...
    def _resolve_task_resources(self, task: ExecutionTask) -> list[str]:
        ...
        if isinstance(raw_resources, list):
            return sorted({resource.strip() for resource in raw_resources if isinstance(resource, str) and resource.strip()})
        raw_capabilities = task.payload.get("required_capabilities", [])
        if not isinstance(raw_capabilities, list):
            return []
        return sorted({f"capability:{name.strip()}" for name in raw_capabilities if isinstance(name, str) and name.strip()})

    def _build_resource_lock_failure(self, task: ExecutionTask, lock_info: dict[str, Any]) -> ExecutionResult:
        timestamp = datetime.now(timezone.utc)
    ...
    def _attach_lock_details(
        ...
        result.details["resource_lock"] = {
            ...
            "release_reason": release_info.get("release_reason") if release_info else None,
            "quarantine_until": release_info.get("quarantine_until") if release_info else None,
        }
