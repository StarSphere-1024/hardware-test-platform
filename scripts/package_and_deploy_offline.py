...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("host", nargs="?", default=os.getenv("REMOTE_HOST", "192.168.100.91"))
    parser.add_argument("user", nargs="?", default=os.getenv("REMOTE_USER", "seeed"))
    parser.add_argument("password", nargs="?", default=os.getenv("REMOTE_PASS", "seeed"))
    parser.add_argument("remote_dir", nargs="?", default=os.getenv("REMOTE_DIR", "/home/seeed/hardware_test"))
    parser.add_argument(
        "--wheelhouse",
        default=None,
    ...
    parser.add_argument(
        "--with-psutil",
        action="store_true",
        help="also prepare psutil source package for richer remote dashboard metrics",
    )
    parser.add_argument(
        "--skip-venv",
        action="store_true",
...
def ensure_dependency_artifacts(wheelhouse: Path, *, download_missing: bool, with_psutil: bool) -> None:
    ...
    if with_psutil and not any(name.startswith("psutil-") for name in artifact_names):
...
def deploy_to_remote(
    *,
    ...
    remote_workspace = f"{remote_dir}/workspace"
    ...
    run_command(
        ssh_base_args(host, user)
        + [f"mkdir -p '{remote_dir}' && rm -rf '{remote_workspace}' && mkdir -p '{remote_workspace}'"],
        env=env,
    )
    ...
    remote_setup_lines = [
        ...
        f"tar -xzf '{remote_source_bundle}' -C '{remote_workspace}'",
    ]
    if not skip_deps:
    ...
    if not skip_venv:
        ...
        remote_setup_lines.append(f"test -x '{remote_venv}/bin/python'")
    if not skip_deps:
        ...
        if with_psutil:
            remote_setup_lines.extend(
                [
                    ...
                    f"  '{remote_venv}/bin/pip' install --no-index --find-links='{remote_wheelhouse}' {quote_args(OPTIONAL_DEPENDENCIES)} || true",
                    "else",
...
def main() -> int:
    ...
    try:
        ...
        print(f"[INFO] Remote workspace: {args.remote_dir}/workspace")
