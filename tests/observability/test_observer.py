...
def test_scheduler_writes_events_snapshots_logs_and_reports(tmp_path: Path) -> None:
    ...
    resolved = resolver.resolve_fixture(
        ...
        request={
            ...
            "fixture_path": "fixtures/linux_host_pc.json",
        },
    )
    ...
    report_json = next(item for item in reports if item.suffix == ".json")
...
def test_observer_writes_live_case_summaries_before_fixture_finishes(
    tmp_path: Path,
) -> None:
    resolver = ConfigResolver(REPO_ROOT)
    resolved = resolver.resolve_fixture(
    ...
    function_task = next(
        ...
        if task.parent_task_id == case_task.task_id and task.task_type == "function"
    )
    ...
    observer.on_task_started(
        function_task, plan_id=plan.plan_id, attempt=1, status_before="pending"
    )

    snapshot_payload = json.loads(
        (tmp_path / "tmp" / "req-obs-live-001_snapshot.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_observer_and_report_preserve_timeout_residual_risk(tmp_path: Path) -> None:
    ...
    root_result = Scheduler(FunctionExecutor({"test_eth_ping": blocking_timeout})).run(
        plan, context
    )

    snapshot_payload = json.loads(
        result_store.snapshot_path("req-obs-timeout-001").read_text(encoding="utf-8")
    )
    event_lines = (
        event_store.event_log_path("req-obs-timeout-001")
    ...
    assert (
        function_result.details["residual_risk"]["kind"]
        == "timeout_background_execution_unknown"
    )
    assert (
        snapshot_payload["results"][0]["children"][0]["children"][0]["details"][
            "residual_risk"
        ]["kind"]
        == "timeout_background_execution_unknown"
    )
    ...
    timeout_event = next(
        entry["event"]
        for entry in events
        if entry["event"]["event_type"] == "task_finished"
        and entry["event"].get("task_name") == "test_eth_ping"
    )
    assert (
        timeout_event["payload"]["residual_risk"]["kind"]
        == "timeout_background_execution_unknown"
    )
    assert (
        report_payload["root_result"]["children"][0]["children"][0]["details"][
            "residual_risk"
        ]["kind"]
        == "timeout_background_execution_unknown"
    )
    ...
    assert (
        "test_eth_ping: timeout returned before the worker could be confirmed stopped"
        in report_text.read_text(encoding="utf-8")
    )
